...
public class FacturaController{
	...
	public String crear(@PathVariable(value = "clienteId") Long clienteId, Map<String, Object> model,
		...
		model.put("proveedores",proveedores.findAll());
	...
	public String guardar(@Valid Factura factura, 
		...
		if (result.hasErrors()) {
			...
			model.addAttribute("proveedores",proveedores.findAll());
			return "factura/form";
		}
		...
		if (!foto.isEmpty()) {
			...
			if (factura.getId() != null && factura.getId() > 0 && factura.getFoto() != null
					&& factura.getFoto().length() > 0) {

				uploadFileService.delete(factura.getFoto());
			}
			...
			factura.setFoto(uniqueFilename);
		}
		...
		return "redirect:/facturas/form/"+ factura.getCliente().getId();
	}
	@Secured("ROLE_ADMIN")
 ...
 Page<Factura> factura =  clienteService.findByClienteAndProveedorAndLugarAndEmviadoN(desde,hasta,cliente, proveedor, lugar, pageRequest);
 ...
 //captura la candidad buscada en el filtro
 	model.addAttribute("cantidad",clienteService.findByClienteAndProveedorAndLugarAndEmviadoN(desde,hasta,cliente, proveedor, lugar, pageable).getNumberOfElements());
	...
	Pageable pageRequest = PageRequest.of(page, 10);
	 ...
	 //captura la candidad buscada en el filtro
	 	model.addAttribute("cantidad",clienteService.findByClienteAndProveedorAndLugarAndEmviadoS(desde,hasta,cliente, proveedor, lugar, pageable).getNumberOfElements());
